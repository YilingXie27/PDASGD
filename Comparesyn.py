"""
This code is to compare PDASGD(our algorithm) with AAM, APDAGD,PDASMD,Sinkhorn, stochastic Sinkhorn  on synthetic data. 
...
###############################
#%% 
def synthetic_img_input(m, fraction_fg):
    fg_max_intensity = 10;
    bg_max_intensity = 1;

    IMG = bg_max_intensity * np.random.uniform(0, 1, (m,m))

    img_i = np.random.choice(m)
    img_j = np.random.choice(m)
    length_fg_side = int(np.floor(m * np.sqrt(fraction_fg)))
    for i in range(length_fg_side):
        for j in range(length_fg_side):
            ind_i = ((img_i + (i + 1) ) % m) -1
            if ind_i == 0:
                ind_i = m -1
            ind_j = ((img_j + (j+1)) % m) -1
            if ind_j == 0:
                ind_j = m -1       
            IMG[ind_i, ind_j]=fg_max_intensity * np.random.uniform(0,1)
    return IMG 

fraction_fg= 0.2
def presyn(synthetic):
    synthetic/=synthetic.sum()
...
for i in range(5):
    m = 3
    p_list_pre.append( presyn( synthetic_img_input(m, fraction_fg)  ) )
    q_list_pre.append( presyn( synthetic_img_input(m, fraction_fg)  ) )
...
###################################
# Compare on the synthetic dataset#
################################### 
#%% Sinkhorn
...
plt.savefig("Deternsyn4.pdf", format="pdf")
